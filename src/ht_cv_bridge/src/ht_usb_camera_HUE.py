...
for i in range(0,11):
    ...
    #亮度
    c_10=cap.set(cv2.CAP_PROP_BRIGHTNESS,0.1)
    # #对比度
    c_11=cap.set(cv2.CAP_PROP_CONTRAST,0.5)
    # #饱和度
    c_12=cap.set(cv2.CAP_PROP_SATURATION,0)
    # cap.set(cv2.CAP_PROP_SHARPNESS,255)
    # #色相
    cap.set(cv2.CAP_PROP_HUE,i*0.1)
    # #增益
    # c_14=cap.get(cv2.CAP_PROP_GAIN)
    # #曝光
    ...
    cv2.imwrite('HUE/HUE:{}_1080.png'.format(c_13), frame)
