...
class Navigate2C(smach.State):
    ...
    def get_waypoints(self):
        waypoints = [
            ...
            [[-0.023, -2.643, 0.000],[0.000, 0.000,  0.149, 0.989]]            #C区目标点1
    ...
    def listener(self):
        add_thread = threading.Thread(target = thread_dis_c)
        add_thread.start()
    # rospy.init_node('listen', anonymous=True)

        if not rospy.is_shutdown():
            pose = rospy.Subscriber("/move_base/feedback",MoveBaseActionFeedback,dis_cb_c,queue_size=1)
...
def dis_cb_c(pose):
    ...
    if distance <= 0.8 and (0 < angle < 90) and dis_flag_c:
        dis_flag_c = False
        for i in range(2):
            ht_save_image_c(i)
            # rospy.sleep(0.2)            
...
def ht_save_image_c(i):
    ...
    frame = bridge.imgmsg_to_cv2(data, "bgr8")
        # cv2.rectangle(frame, (180, 160), (460, 320), (0, 255, 0), 2)
    # cv2.imwrite('/home/ucar/ROS_xunfei/ucar_ws/src/game_start/scripts/x_6_{}.jpg'.format(i), frame)
    cv2.imwrite('/home/ucar/ROS_xunfei/ucar_ws/src/ht_image/scripts/xunfei_detect_img/x_c_1_{}.jpg'.format(i), frame)
    return True
...
class Aruco(smach.State):
    ...
    def execute(self, userdata):
        ...
        code = ht_aruco()
        if code == 0:
            userdata.navpoints = 2          #目标点D1  navpoints=0           D2  navpoints=1     D3  navpoints=2
            # return 'nav2D1'    
            client.call('蔬菜','xiaoyan')        
            return 'NAV2C'
        elif code == 1:
            userdata.navpoints = 2        #抽签后修改
            # return 'nav2D2'            
...
def thread_detect():
    os.system('python3 /home/ucar/ROS_xunfei/ucar_ws/src/ht_image/scripts/xunfei_e_b_detect_m.py') #c区持续检测
