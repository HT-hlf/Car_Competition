...
def ht_glass_pb(ht_list,xywh):
    if ht_list[0]<=xywh[0]<=ht_list[2] and ht_list[1]<=xywh[1]<=ht_list[3]:
        return True
    else:
        return False
#plot_one_box(xyxy, im0, label=label, color=colors[int(cls)], line_thickness=3)
def ht_plot_one_box(x, img, color=None, label=None, line_thickness=None):
    # Plots one bounding box on image img
    # glass_longhairhead
    # glass_shorthairhead
    # noglass_longhairhead
    # noglass_shorthairhead
    # name=['glass_longhairhead','glass_shorthairhead','noglass_longhairhead','noglass_shorthairhead']
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
