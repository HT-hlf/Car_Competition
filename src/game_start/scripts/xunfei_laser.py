...
def listener():
    ...
    sum_people_list.append(people_list)
    if len(sum_people_list)>3:
        # print(sum_people_list)
        for list in sorted(sum_people_list, key=lambda x: len(x),reverse=True)[0:2]:
            #这里可以设置过滤
    ...
    elif len(sum_people_list)!=0:
        for list in sorted(sum_people_list, key=lambda x: len(x),reverse=True):
            #这里可以设置过滤
