...
for i in range(0,10):
    ...
    #亮度
    c_10=cap.set(cv2.CAP_PROP_BRIGHTNESS,i*0.1)
    # #对比度
    c_11=cap.set(cv2.CAP_PROP_CONTRAST,0)
    # #饱和度
    c_12=cap.set(cv2.CAP_PROP_SATURATION,0)
    ...
    # cap.set(cv2.CAP_PROP_SHARPNESS,255)
    cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0.25)
    c_15 = cap.set(cv2.CAP_PROP_EXPOSURE, 0.22)
    ...
    cv2.imwrite('BRIGHTNESS/BRIGHTNESS:{}.png'.format(c_10), frame)
