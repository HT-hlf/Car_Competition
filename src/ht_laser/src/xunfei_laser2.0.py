...
def listener():
    ...
    for r in data.ranges:
        ...
        abs_x = x + car_x
        abs_y = y + car_y
        if abs_x < map_people_th or abs_x > 150-map_people_th or abs_y > 260 or abs_y < car_y:
            abs_x = car_x
            abs_y = car_y
    ...
    for result in sorted(result_list,key=lambda x:x[7]):
        select_result_list.append(result[0:7])
        print(result[7])
