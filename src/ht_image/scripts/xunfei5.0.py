...
# def ht_multi_callback(image,ht_d):
#     print('waiting')
#     if ht_d.detect:
#         print('...')
...
def ht_d_callback(ht_d):
    # global info_pub,image_pub

    if ht_d.detect:
        print('start detect')
        rospy.Subscriber("/ht_image_biaoding_view/ht_image_biaoding_raw", Image, ht_image_callback, queue_size=1,
                         buff_size=52428800)
...
if __name__ == '__main__':
    ...
    #model = torch.load('/home/ucar/ROS_xunfei/ucar_ws/src/ht_image/scripts/weights/xunfei_best.pt', map_location=device)[
        # 'model'].float()  # load to FP32
    model = torch.load('/home/ucar/ROS_xunfei/ucar_ws/src/ht_image/scripts/weights/274.pt', map_location=device)['model'].float()  # load to FP32
    model.to(device).eval()
    model.half()  # to FP16
    # for i in range(5):
    #     img = torch.ones((1, 3, imgsz, imgsz), device=device)  # init img
    #     # _ = model(img.half())  # run once
    #     pred = model(img.half(), augment=False)[0]
